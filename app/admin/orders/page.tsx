...
import { 
  ...
  Clock, CheckCircle, XCircle, Truck, User, Phone, MapPin, CreditCard
} from "lucide-react";
  ...
  return (
    <div>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        ...
        <div className="flex bg-[#1a1a1a] p-1 rounded-xl border border-white/10">
            <button 
                ...
                className={`px-4 py-2 rounded-lg text-sm font-bold transition ${filterStatus === "all" ? "bg-white text-black" : "text-gray-400 hover:text-white"}`}
            >
                Всі
            </button>
            <button 
                onClick={() => setFilterStatus("new")}
                className={`px-4 py-2 rounded-lg text-sm font-bold transition ${filterStatus === "new" ? "bg-blue-600 text-white" : "text-gray-400 hover:text-white"}`}
            >
                Нові
            </button>
            <button 
                onClick={() => setFilterStatus("processing")}
                className={`px-4 py-2 rounded-lg text-sm font-bold transition ${filterStatus === "processing" ? "bg-yellow-600 text-white" : "text-gray-400 hover:text-white"}`}
            >
                В обробці
            </button>
        </div>
      ...
      <div className="space-y-4">
        {loading ? <p className="text-gray-500">Завантаження замовлень...</p> : orders.length === 0 ? (
                ...
                <div key={order.id} className={`bg-[#1a1a1a] border rounded-xl overflow-hidden transition ${expandedId === order.id ? "border-blue-500 ring-1 ring-blue-500" : "border-white/5 hover:border-white/20"}`}>
                    ...
                    <div className="p-4 md:p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 cursor-pointer" onClick={() => toggleExpand(order.id)}>
                        ...
                        <div className="flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-8 flex-1 md:justify-end w-full md:w-auto">
                            ...
                            <div 
                                onClick={(e) => e.stopPropagation()} // Щоб не відкривало/закривало при кліку на селект
                                className="relative"
                            >
                                {/* @ts-ignore */}
