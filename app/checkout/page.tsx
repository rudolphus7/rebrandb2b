...
export default function CheckoutPage() {
    ...
    useEffect(() => {
        const loadUserData = async () => {
            ...
            if (session?.user) {
                ...
                const { data: orders } = await supabase.from('orders').select('total_price').eq('user_email', session.user.email);
    ...
    if (cart.length === 0) return <div className="min-h-screen bg-[#111] flex flex-col items-center justify-center text-white p-6"><ShoppingBag size={64} className="text-gray-600 mb-4"/><h1 className="text-3xl font-bold mb-2">Ваш кошик порожній</h1><Link href="/" className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-xl mt-4 transition"><ArrowLeft size={16} className="inline mr-2"/> Перейти до каталогу</Link></div>;
        ...
        <div className="min-h-screen bg-[#111] text-white font-sans pb-20">
            <div className="bg-[#1a1a1a] border-b border-white/10 py-4 sticky top-0 z-50">
                <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
                    ...
                    <button onClick={() => router.push('/')} className="text-gray-400 hover:text-white flex items-center gap-2 text-sm font-bold"><ArrowLeft size={16}/> На головну</button>
                </div>
            </div>

            <main className="max-w-7xl mx-auto px-6 py-12">
                <div className="flex items-center justify-between mb-10">
                    <h1 className="text-4xl font-black tracking-tight flex items-center gap-4 text-white"><Truck size={36} className="text-blue-500"/> Оформлення замовлення</h1>
                </div>
                            ...
                            <div className="space-y-2 relative">
                                ...
                                {isCityDropdownOpen && citySearch.length > 0 && (
                                    <div className="absolute top-full left-0 w-full bg-zinc-900 border border-white/10 rounded-lg max-h-48 overflow-y-auto z-10 shadow-lg">
                                        {filteredCities.length > 0 ? filteredCities.map(city => (<div key={city} onMouseDown={() => handleCitySelect(city)} className="p-3 text-sm hover:bg-blue-600/50 cursor-pointer transition">{city}</div>)) : <div className="p-3 text-sm text-gray-500">Місто не знайдено</div>}
                                    </div>
                                )}
                            </div>
                                ...
                                {cart.map((item, index) => (
                                    <div key={index} className="flex justify-between text-sm text-gray-400 items-start">
                                        <div className="flex flex-col max-w-[70%]">
                                            ...
                                            <span className="text-xs text-gray-500">{item.selectedSize ? `Розмір: ${item.selectedSize}` : ''} | x{item.quantity}</span>
                                        </div>
    ...
    return (
        <div className="space-y-2">
            ...
            <div className="relative">
                ...
                <input type={name === 'phone' ? 'tel' : name === 'email' ? 'email' : 'text'} name={name} value={value} onChange={onChange} disabled={disabled} className="w-full bg-black/50 border border-white/10 rounded-lg py-3 pl-10 pr-4 text-white focus:border-blue-500 outline-none disabled:opacity-50 disabled:cursor-not-allowed" required={required} />
            </div>
        </div>
    );
}
...
function PaymentOption({ id, name, value, label, checked, onChange, icon: Icon }: any) {
    return (
        <label htmlFor={id} className={`flex items-center p-4 rounded-xl cursor-pointer transition duration-200 border-2 ${checked ? 'bg-blue-600/20 border-blue-600' : 'bg-black/50 border-white/10 hover:border-white/30'}`}>
            ...
            <div><p className="font-bold text-white">{label}</p><p className="text-xs text-gray-400 mt-1">{value === 'invoice' ? 'Ви отримаєте рахунок для оплати.' : 'Безпечна оплата онлайн.'}</p></div>
        </label>
    );
}