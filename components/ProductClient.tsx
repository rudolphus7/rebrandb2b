    ...
    <div className="bg-[#FAFAFA] min-h-screen py-10 font-sans text-[#111] antialiased">
      ...
      <div className="container mx-auto px-4 max-w-[1440px]">
        ...
        <div className="text-xs font-medium text-gray-400 mb-8 flex items-center gap-3">
            <Link href="/" className="hover:text-black transition">Головна</Link> <ChevronRight size={14} className="text-gray-300"/>
            <Link href="/catalog" className="hover:text-black transition">Каталог</Link> <ChevronRight size={14} className="text-gray-300"/>
            <span className="font-semibold text-black line-clamp-1">{product.title}</span>
        </div>

        {/* === ВЕРХНІЙ БЛОК === */}
        <div className="bg-white rounded-[32px] p-8 lg:p-12 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col lg:flex-row gap-16 mb-12">
          ...
          <div className="w-full lg:w-[50%] flex flex-col gap-10">
            ...
            <div className="flex gap-6 h-[550px]">
                ...
                <div className="flex-1 bg-[#F8F8F8] rounded-[32px] flex items-center justify-center p-10 relative overflow-hidden group">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={mainImage} alt={product.title} className="w-full h-full object-contain transition-transform duration-500 group-hover:scale-105 mix-blend-multiply"/>
            ...
            {uniqueColors.length > 1 && (
                <div>
                    <p className="text-base font-bold text-gray-900 mb-4 flex items-center gap-3">
                        Колір: 
                        <span className="font-medium text-gray-700 bg-gray-100 px-3 py-1 rounded-full text-sm">{selectedColor}</span>
                    </p>
          ...
          <div className="w-full lg:w-[50%] flex flex-col relative">
             ...
             <div className="mb-8">
                 <h1 className="text-4xl lg:text-5xl font-black text-[#111] mb-3 leading-tight tracking-tight">
                    {product.title}
                 </h1>
                         ...
                         return (
                            <div key={variant.id} className={`flex items-center justify-between p-5 transition-all duration-200 ${qty > 0 ? 'bg-blue-50/50' : 'hover:bg-[#FAFAFA]'}`}>
                                 ...
                                 <div className="w-[25%] text-center flex flex-col items-center justify-center">
                                     {isAvailable ? (
                                        <>
                                            <span className="text-base font-bold text-[#111]">{variant.available}</span>
                                            <span className="text-[10px] text-green-600 font-bold uppercase tracking-wider mt-0.5">На складі</span>
                                        </>
                                     ) : (
                                 ...
                                 <div className="w-[40%] flex justify-end">
                                     <div className={`flex items-center bg-[#F3F4F6] rounded-full p-1 w-36 shadow-sm transition-all ${isAvailable ? '' : 'opacity-40 pointer-events-none'}`}>
                                         ...
                                         <input 
                                            ...
                                            className="w-full h-full text-center bg-transparent font-bold text-lg focus:outline-none text-[#111] px-2"
                                            placeholder="0"
                                         />
                                         <button 
                                            onClick={() => handleQuantityChange(variant.id, 1, variant.available)}
                                            className="w-9 h-9 flex items-center justify-center bg-[#111] hover:bg-black/90 text-white rounded-full shadow-sm transition-all active:scale-95"
                                         >
             ...
             {/* ТАБИ (Сучасні) */}
             <div className="mt-12">
                <div className="flex p-1 bg-gray-100 rounded-2xl mb-6 w-fit">
                    <button 
                        onClick={() => setActiveTab('features')} 
                        className={`px-6 py-3 text-sm font-bold rounded-xl transition-all ${activeTab === 'features' ? 'bg-white text-black shadow-sm' : 'text-gray-500 hover:text-black'}`}
                    >
                        Характеристики
                    </button>
                    <button 
                        onClick={() => setActiveTab('description')} 
                        className={`px-6 py-3 text-sm font-bold rounded-xl transition-all ${activeTab === 'description' ? 'bg-white text-black shadow-sm' : 'text-gray-500 hover:text-black'}`}
                    >
                        Опис товару
                    </button>
                </div>

                <div className="text-base text-gray-700 leading-relaxed bg-white p-8 rounded-[24px] border border-gray-100 shadow-[0_4px_20px_rgb(0,0,0,0.03)]">
                    {activeTab === 'features' ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
                            ...
                            {Object.entries(product.specifications || {}).map(([key, value]) => (
                                <div key={key} className="flex justify-between py-3 border-b border-gray-100">
                                    ...
                                    <span className="font-bold text-[#111]">{value as string}</span>
                                </div>
                            ))}
                        </div>
                    ) : (
        ...
        <div className="bg-white rounded-[32px] p-8 lg:p-12 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden border border-gray-100/50">
            <h3 className="text-3xl font-black text-[#111] mb-10">Загальна наявність на складах</h3>
                ...
                <table className="w-full text-left border-collapse min-w-[900px]">
                    <thead>
                        <tr>
                            ...
                            {allSizes.map(size => (
                                <th key={size} className="py-5 font-bold text-[#111] text-center text-sm w-24 border-b-2 border-gray-100 bg-[#FAFAFA]">
                                    {size}
                                </th>
                            ))}
                        </tr>
                        ...
                        {uniqueColors.map(color => {
                            ...
                            return (
                                <tr 
                                    key={color} 
                                    onClick={() => setSelectedColor(color)} 
                                    className={`group cursor-pointer transition-all duration-300 ${isCurrentRow ? 'bg-blue-50/40' : 'hover:bg-[#FAFAFA]'}`}
                                >
                                    <td className="py-5 pl-6 pr-6">
                                        <div className="flex items-center gap-5">
                                            ...
                                            <div className={`font-bold text-base transition-colors ${isCurrentRow ? 'text-blue-700' : 'text-[#111]'}`}>{color}</div>
                                        </div>
                                    </td>
                                    ...
                                    {allSizes.map(size => {
                                        ...
                                        return (
                                            <td key={size} className="py-5 text-center align-middle">
                                                {available > 0 ? (
                                                    <div className={`text-base font-bold transition-all ${inputQty > 0 ? 'text-blue-600 scale-125' : 'text-[#111] group-hover:scale-110'}`}>
                                                        {available}
                                                    </div>
                                                ) : (
